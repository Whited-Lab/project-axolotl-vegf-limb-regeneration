...
```{r}
# Load data
data_dir <- "/n/whited_lab/Users/hsinger/illumina/018_illumina/4_alignment/"
...
subdir_names <- sapply(sample_sheet$Identifier, function(x) paste0("4_alignment/",x), USE.NAMES = F)

files <- file.path(data_dir, sample_sheet$Identifier, "abundance.h5")

tx2gene <- read.csv("AmexT_v47-AmexG_v6.0-DD_t2g.tsv", sep = "\t", header = T)
...
#saveRDS(res_g,file = "018_Illumina_res_g.rds")
# save the Deseq2 DataSet
#saveRDS(res_g_all,file = "018_Illumina_res_g_all.rds")
...
gene_list <- read.table("volcano_gene_amexid.txt", header = FALSE, sep = "\t")
gene_names <- read.table("volcano_gene_names.txt", header = FALSE, sep = "\t")
...
#ggsave("Illumina_018_volcano_control_vs_av951.pdf", plot = v_g, device = "pdf", width = 10, height = 7)
...
gene_list_v3 <- read.csv("heatmap_gene_list_v3.csv") %>%
  select(1:1) 
...
#ggsave("heatmap_full_v5_highlight.pdf", plot = heatmap_g_full, device = "pdf")
...
#write.csv(highlighted_genes_order, "highlighted_genes_order_v5.csv", row.names = FALSE)


# Save the result object as an Excel sheet
write_xlsx(result, "deseq_result_sig.xlsx")
...
#ggsave(PCA.pdf"), plot = pca_g, device = "pdf", width = 5, height = 5)
```
...
#ggsave(("PCA_loadings_pc1.pdf"), plot = loadings_pc1_g, device = "pdf", height = 5, width = 10)
...
#ggsave(("PCA_loadings_pc2.pdf"), plot = loadings_pc2_g, device = "pdf", height = 5, width = 10)
